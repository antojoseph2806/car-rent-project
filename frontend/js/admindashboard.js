...
async function fetchBookingChartData() {
    try {
        const response = await fetch('http://localhost:5000/api/admin/dashboard/charts/bookings', {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${getAuthToken()}`  // Include token in headers
            }
...
async function fetchVehicleUsageChartData() {
    try {
        ...
        const response = await fetch('http://localhost:5000/api/admin/dashboard/charts/vehicles/usage', {
            method: 'GET',
            headers: {
